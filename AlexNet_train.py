...
dropout_rate = 0.5
momentum_rate = 0.9
log_save_path = 'log/Alexnet'
model_save_path = 'models/Alexnet'
...
if __name__ == '__main__':
    ...
    # build_network
    W_conv1 = tf.get_variable('conv1', shape=[11, 11, 3, 96], initializer=tf.contrib.keras.initializers.he_normal())
    b_conv1 = bias_variable([96])
    output = tf.nn.relu(batch_norm(conv2d(x, W_conv1) + b_conv1))
    output = max_pool(output, 2, 2, "pool1")

    W_conv2 = tf.get_variable('conv2', shape=[5, 5, 96, 256], initializer=tf.contrib.keras.initializers.he_normal())
    b_conv2 = bias_variable([256])
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv2) + b_conv2))
    output = max_pool(output, 2, 2, "pool2")

    W_conv3 = tf.get_variable('conv3', shape=[3, 3, 256, 384], initializer=tf.contrib.keras.initializers.he_normal())
    b_conv3 = bias_variable([384])
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv3) + b_conv3))
    output = max_pool(output, 2, 2, "pool3")

    W_conv4 = tf.get_variable('conv4', shape=[3, 3, 384, 384], initializer=tf.contrib.keras.initializers.he_normal())
    b_conv4 = bias_variable([384])
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv4) + b_conv4))
    output = max_pool(output, 2, 2)

    W_conv5 = tf.get_variable('conv5', shape=[3, 3, 384, 256], initializer=tf.contrib.keras.initializers.he_normal())
    b_conv5 = bias_variable([256])
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv5) + b_conv5))
    output = max_pool(output, 2, 2)

    # output = tf.contrib.layers.flatten(output)
    output = tf.reshape(output, [-1, 1*1*256])

    W_fc1 = tf.get_variable('fc1', shape=[256, 4096], initializer=tf.contrib.keras.initializers.he_normal())
